...
Student::Student(int id, string name, string pwd)//有参构造函数
{
	//初始化属性
	this->m_Id = id;
	this->m_Name = name;
...
void Student::showMyOrder()//查看我的预约
{
	...
	for (int i = 0; i < of.m_Size; i++)
	{
		if (atoi(of.m_orderData[i]["stuId"].c_str()) == this->m_Id)
		{
			...
			string status = "  状态：";//0 取消；1 审核中；2 已预约；-1 预约失败
...
void Student::cancelOrder()//取消预约
{
	...
	cout << "审核中或者预约成功的可以取消，请输入待取消记录：" << endl;
